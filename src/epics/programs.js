        ...
        return getD2()
            .then(d2 =>
                d2.models.programs.list({
                    fields:
                        'id,displayName~rename(name),programStages[id,displayName~rename(name)]',
                    paging: false
                })
            )
...
export const loadProgramIndicators = action$ =>
    action$.ofType(types.PROGRAM_INDICATORS_LOAD).concatMap(action =>
        getD2()
            ...
            .then(program =>
                setProgramIndicators(action.programId, sortBy('name', program.programIndicators))
            )
    ...
    action$.ofType(types.PROGRAM_ATTRIBUTES_LOAD).concatMap(action =>
        getD2()
            ...
            .then(program =>
                setProgramAttributes(
                    action.programId,
                    program.programTrackedEntityAttributes.map(d => d.trackedEntityAttribute)
                )
            )
