...
export const saveNewReport = (action$, store) =>
  action$.ofType(types.REPORT_SAVE_NEW).concatMap(() => {
    ...
    const { programStageDataElements, programTrackedEntityAttributes, optionSets, form } = state;
    const { program, orgUnits, programStages, period, startDate, endDate, filters } = form;
    const attributes = programTrackedEntityAttributes[program].filter(item => item.id !== 'HAZ7VQ730yn') || [];
    ...
    const newAnalyticRequest = programStages.map(prid => {
      return getAnalyticsRequest({ id: program }, { id: prid }, period, startDate, endDate, orgUnits, filters, null);
    });

    return Promise.all(newAnalyticRequest)
      .then(analytics => {
        const mergedAnalytics = getMergedAnalytics(analytics);
        const linelist = transformForTableList(mergedAnalytics, attributes, programdataElements, optionSets, filters);
...
export const getAnalyticsRequest = async (program, programStage, period, startDate, endDate, orgUnits, dataItems) => {
  const d2 = await getD2();

  let analyticsRequest = new d2.analytics.request().withProgram(program.id).withStage(programStage.id);
  ...
  const tableRows = rows.map((row, index) =>
    row.reduce((obj, value, index) => {
      ...
      if (dataItems.includes(header.name)) {
        ...
        const valueLength =
          obj[header.name].length > header.column.length ? obj[header.name].length : header.column.length;
        const size = columnsSize[header.name] > valueLength ? columnsSize[header.name] : valueLength;
