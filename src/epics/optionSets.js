...
import { addOptionSet, addAllOptionSets } from '../actions/optionSets';
...
export const loadAllOptionSets = action$ =>
    action$.ofType(types.OPTION_SET_LOAD_ALL).concatMap(() => {
        return getD2()
            .then(d2 =>
                d2.models.optionSets.list({
                    fields:
                        'id,displayName~rename(name),version,options[code,displayName~rename(name)]',
                    paging: false
                })
            )
            .then(optionSets => addAllOptionSets(optionSets.toArray()))
            .catch(errorActionCreator(types.PROGRAMS_LOAD_ERROR));
    });

export default combineEpics(loadOptionSet, loadAllOptionSets);
