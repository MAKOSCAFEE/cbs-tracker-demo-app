...
class GroupEditor extends Component {
    ...
    if (props.hasOwnProperty('filterText') && this.leftSelect && this.rightSelect) {
      this.setState({
        selectedLeft: [].filter.call(
          ...
          item => item.text.toLowerCase().indexOf(`${props.filterText}`.trim().toLowerCase()) !== -1
        ).length,
  ...
  getItemStoreIsCollection() {
    return (
      ...
      (typeof this.props.itemStore.state.values === 'function' && typeof this.props.itemStore.state.has === 'function')
    );
  }
  getItemStoreIsArray() {
  ...
  render() {
    ...
    const selectedLabel = () =>
      this.getSelectedCount() > 0 ? `${this.getSelectedCount()} ${this.getTranslation('selected')}` : '';
    ...
    return (
      <div style={styles.container}>
        <div style={styles.left}>
          <Paper style={styles.paper}>
            ...
            >
              {this.getAvailableItemsFiltered().map(item => (
                <option key={item.value} value={item.value} onDoubleClick={this.onAssignItems} style={styles.options}>
                  {item.text}
                </option>
              ))}
            </select>
          ...
          <RaisedButton
            label={`${assignAll} ${
              this.getAvailableItemsUnfilteredCount() === 0 ? '' : this.getAvailableItemsUnfilteredCount()
            } \u2192`}
        ...
        <div style={styles.middle}>
          ...
          <div style={styles.status}>
            {this.state.loading ? <CircularProgress small style={{ width: 60, height: 60 }} /> : undefined}
          </div>
        </div>
        <div style={styles.right}>
          <Paper style={styles.paper}>
            ...
            >
              {this.getAssignedItemsFiltered()
                ...
                .map(item => (
                  <option key={item.value} value={item.value} onDoubleClick={this.onRemoveItems} style={styles.options}>
                    {item.text}
                  </option>
                ))}
            </select>
          ...
          <RaisedButton
            label={`\u2190 ${this.getTranslation('remove_all')} ${
              this.getAssignedItemsUnfilteredCount() > 0 ? this.getAssignedItemsUnfilteredCount() : ''
            }`}
